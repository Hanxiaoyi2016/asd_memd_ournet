# fork from https://github.com/mariogrune/MEMD-Python-
...
def envelope_mean(m, t, seq, ndir, N, N_dim):  # new
    ...
    for it in range(0, ndir):
        ...
        if N_dim != 3:
            ...
            tht = np.arctan2(
                np.sqrt(np.flipud(np.cumsum(b[:0:-1]**2))), b[:N_dim-1]).transpose()
...
def set_value(*args):
    ...
    # Doesn't do the same as the Matlab script
    if len(q) == 0:
...
def memd(*args):
    x, seq, t, ndir, N_dim, N, sd, sd2, tol, nbit, MAXITERATIONS, stop_crit, stp_cnt = set_value(
        args)
    ...
    while stop_emd(r, seq, ndir, N_dim) == False:
        ...
        if stop_crit == 'stop':
            ...
            counter = 0
            stop_sift, env_mean, counter = fix(
                m, t, seq, ndir, stp_cnt, counter, N, N_dim)
        ...
        while stop_sift == False and nbit < MAXITERATIONS:
            ...
            # computation of mean and stopping criterion
            if stop_crit == 'stop':
                stop_sift, env_mean = stop(
                    m, t, sd, sd2, tol, seq, ndir, N, N_dim)
            else:
                stop_sift, env_mean, counter = fix(
                    m, t, seq, ndir, stp_cnt, counter, N, N_dim)
            ...
            if nbit == (MAXITERATIONS-1) and nbit > 100:
                warnings.wanr(
                    'emd:warning', 'forced stop of sifting : too many erations')
...
if __name__ == '__main__':
    inp = np.random.rand(9950, 53)#3为样本个数，100为特征个数，因此需要转置一下
    imf = memd(inp)
    print(imf, imf.shape)
    imf_x = imf[:, 0, :]  # imfs corresponding to 1st component
    imf_y = imf[:, 1, :]  # imfs corresponding to 2nd component
